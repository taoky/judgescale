/*
   "judgescale" is a slightly modified version of "runguard"
   original source code: https://github.com/DOMjudge/domjudge/blob/d66221b1346372809ff171310b929a1868904b66/judge/runguard.c
...
struct timeval progstarttime, progendtime, starttime, endtime;
...
void output_exit_time(int exitcode, double cpudiff)
{
	...
	write_meta("wall - cpu", "%.3f", walldiff - cpudiff);
...
int main(int argc, char **argv)
{
	...
	outputmeta = 1;
	metafilename = "/dev/stdout";
	outputtimetype = CPU_TIME_TYPE;
	preserve_environment = 0;
	memsize = 128 * 1024 * 1024;  // 128M
	filesize = nproc = RLIM_INFINITY;
	...
	while ( (opt = getopt_long(argc,argv,"+r:u:g:d:t:C:m:f:p:P:co:e:s:EV:M:vq",long_opts,(int *) 0))!=-1 ) {
		...
		case 'r': /* rootdir option */
			error(0, "chroot() func removed");
			break;
		case 'u': /* user option: uid or string */
			error(0, "setuid() func removed");
			break;
		case 'g': /* group option: gid or string */
			error(0, "setgid() func removed");
			break;
		case 'd': /* chdir option */
	...
	default: /* become watchdog */
		...
		if ( gettimeofday(&progendtime,NULL) ) error(errno,"getting time");
		double judgewalldiff = (progendtime.tv_sec  - progstarttime.tv_sec ) +
	           			  	   (progendtime.tv_usec - progstarttime.tv_usec) * 1E-6;
		write_meta("judgetime", "%.3f", judgewalldiff);
